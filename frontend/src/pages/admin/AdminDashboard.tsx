...
function AdminDashboard() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        const [dashStats, auditResponse] = await Promise.all([
          getDashboardStats(),
          getAuditLogs(0, 10),
        ]);
        setStats(dashStats);
        setRecentActivity(auditResponse.items);
      } catch {
        // Gracefully handle API unavailability
        setStats({ users: { total: 0, active: 0 }, resources: { total: 0, active: 0 }, suppliers: { total: 0, active: 0 } });
        setRecentActivity([]);
      } finally {
        setLoading(false);
      }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.statsGrid}>
        {statCards.map((card) => {
          ...
          return (
            <Link key={card.key} to={card.link} style={{ ...styles.statCard, ...(isHovered ? styles.statCardHover : {}) }}
      ...
      <div style={styles.section}>
        ...
        <div style={styles.quickActions}>
          <Link to="/admin/users?action=new" style={{ ...styles.quickActionCard, ...(hoveredCard === 'newUser' ? { transform: 'translateY(-2px)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' } : {}) }}
            onMouseEnter={() => setHoveredCard('newUser')} onMouseLeave={() => setHoveredCard(null)}>
            <div style={{ ...styles.quickActionIcon, backgroundColor: '#eff6ff', color: '#3b82f6' }}><StatIcons.Plus /></div>
            <span style={styles.quickActionText}>Add New User</span>
          </Link>
          <Link to="/admin/resources?action=new" style={{ ...styles.quickActionCard, ...(hoveredCard === 'newResource' ? { transform: 'translateY(-2px)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' } : {}) }}
            onMouseEnter={() => setHoveredCard('newResource')} onMouseLeave={() => setHoveredCard(null)}>
            <div style={{ ...styles.quickActionIcon, backgroundColor: '#ecfdf5', color: '#10b981' }}><StatIcons.Plus /></div>
            <span style={styles.quickActionText}>Add New Resource</span>
          </Link>
          <Link to="/admin/suppliers?action=new" style={{ ...styles.quickActionCard, ...(hoveredCard === 'newSupplier' ? { transform: 'translateY(-2px)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' } : {}) }}
            onMouseEnter={() => setHoveredCard('newSupplier')} onMouseLeave={() => setHoveredCard(null)}>
            <div style={{ ...styles.quickActionIcon, backgroundColor: '#f5f3ff', color: '#8b5cf6' }}><StatIcons.Plus /></div>
        ...
        <div style={styles.activityList}>
          {recentActivity.length === 0 ? (
            ...
            recentActivity.map((activity) => {
              ...
              return (
                <div key={activity.id} style={{ ...styles.activityItem, ...(isHovered ? styles.activityItemHover : {}) }}
                  onMouseEnter={() => setHoveredActivity(activity.id)} onMouseLeave={() => setHoveredActivity(null)}>
                  <div style={{ ...styles.activityIcon, backgroundColor: actionStyle.bg, color: actionStyle.text }}>
                  ...
                  <div style={styles.activityContent}>
                    <div style={styles.activityText}>
                      ...
                      <span style={{ ...styles.actionBadge, backgroundColor: actionStyle.bg, color: actionStyle.text }}>{activity.action}</span>{' '}
                      {activity.entity_type} #{activity.entity_id}
                    </div>
                    <div style={styles.activityMeta}>{formatTimeAgo(activity.created_at)}</div>
                  </div>
                </div>
              );
            })
